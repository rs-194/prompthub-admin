...
- 对话测试 / Prompt 调试台已接入后端 `POST /api/v1/chat-test/run` 真实 LLM 非流式调用，支持选择提示词模板、选择启用模型配置、选择启用中的知识库文档作为 mock context、设置 temperature / maxTokens / outputFormat 测试参数、展示真实 output，并使用后端返回的 record 更新最近测试记录（Prompt / Model / Knowledge 仍是前端配置源；当前不做真实 SSE，不做真实 RAG；非流式阶段暂不支持停止生成）
...
- FastAPI 后端 Phase 2.1 最小骨架、Phase 2.2 TestRecord 持久化接口、Phase 2.3 真实 LLM 非流式调用接口与 Phase 2.4 前端 ChatTest run 接入，包含 CORS、SQLite / SQLAlchemy 基础连接、`/api/v1` 路由入口、`GET /api/v1/health` 健康检查接口、测试记录 CRUD，以及 `POST /api/v1/chat-test/run` 真实 LLM 非流式调用并保存 TestRecord（当前未实现 ChatTest stream、真实 RAG / auth）
- Element Plus 基础接入
...
- 对话测试真实 stream / SSE / fetch stream、参数扩展与测试记录详情能力
- FastAPI 后端接口：ChatTest stream / SSE / fetch stream、真实接口联调、Prompt / Model / Knowledge 后端迁移
...
当前后端包含 Phase 2.1 骨架、Phase 2.2 TestRecord 持久化接口和 Phase 2.3 真实 LLM 非流式调用接口：
...
说明：`LLM_API_KEY` 只允许配置在后端环境变量中，不要提交真实 API Key。当前接口为非流式调用，不是真实 RAG；`modelName` 当前只用于测试记录展示字段，真实调用使用后端 `LLM_MODEL`。

## 目录结构说明
```text
prompthub-admin/
├─ backend/                      # FastAPI 后端骨架、TestRecord 持久化与真实 LLM 非流式调用（Phase 2.1 / 2.2 / 2.3）
│  ├─ app/
...
## 项目状态说明
- 当前状态：开发中（前端部分 mock + 后端真实 run 接口接入阶段）
- 页面状态：部分页面为骨架页面，业务功能待实现
- 数据状态：Prompt / Model / Knowledge 等前端业务数据仍为 mock；ChatTest 运行已接入后端真实 LLM 非流式 run 接口，并使用后端返回的 record 更新最近测试记录
- 后端状态：FastAPI 最小骨架、TestRecord CRUD 与 ChatTest 真实 LLM 非流式调用接口已建立；前端已接入该 run 接口，ChatTest stream、真实 RAG、真实认证仍待实现
- 项目容器化：计划中，后续考虑补充 Dockerfile 与 docker-compose 配置

## 当前限制
- 当前页面以骨架和布局为主，暂未实现完整业务交互。
- 当前 Prompt / Model / Knowledge 等业务数据仍未接入真实后端业务接口；ChatTest run 已接入后端真实 LLM 非流式接口。
- 当前仅支持 mock 登录与前端路由守卫，暂无真实后端鉴权、RBAC 和接口鉴权。
- 当前后端已实现真实 LLM 非流式调用最小闭环，前端 ChatTest 已接入该 run 接口；当前未实现 ChatTest stream、真实 RAG 或真实认证；Prompt / Model / Knowledge 模块仍未迁移到后端。
- 当前暂未接入 Docker 部署流程。
