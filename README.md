...
- FastAPI 后端 Phase 2.1 最小骨架、Phase 2.2 TestRecord 持久化接口与 Phase 2.3 真实 LLM 非流式调用接口，包含 CORS、SQLite / SQLAlchemy 基础连接、`/api/v1` 路由入口、`GET /api/v1/health` 健康检查接口、测试记录 CRUD，以及 `POST /api/v1/chat-test/run` 真实 LLM 非流式调用并保存 TestRecord（当前未接前端，未实现 ChatTest stream、真实 RAG / auth）
- Element Plus 基础接入
...
- FastAPI 后端接口：前端接入 ChatTest 真实运行接口、ChatTest stream / SSE / fetch stream、真实接口联调、Prompt / Model / Knowledge 后端迁移
...
- 后端状态：FastAPI 最小骨架、TestRecord CRUD 与 ChatTest 真实 LLM 非流式调用接口已建立；前端尚未接入该接口，ChatTest stream、真实 RAG、真实认证仍待实现
...
- 当前后端已实现真实 LLM 非流式调用最小闭环，但前端尚未接入；当前未实现 ChatTest stream、真实 RAG 或真实认证；Prompt / Model / Knowledge 模块仍未迁移到后端。
- 当前暂未接入 Docker 部署流程。
