...
- FastAPI 后端 Phase 2.1 最小骨架，包含 CORS、SQLite / SQLAlchemy 基础连接、`/api/v1` 路由入口和 `GET /api/v1/health` 健康检查接口（当前未实现 TestRecord CRUD、ChatTest stream、真实 LLM / RAG / auth）
- Element Plus 基础接入
...
- FastAPI 后端接口：TestRecord 持久化、ChatTest stream、真实接口联调
...
```text
prompthub-admin/
├─ backend/                      # FastAPI 后端骨架（Phase 2.1）
│  ├─ app/
...
- 数据状态：前端业务数据仍为 mock，TestRecord 尚未持久化
- 后端状态：FastAPI 最小骨架已建立，仅提供 health check；业务接口待实现
...
- 当前后端未实现 TestRecord CRUD、ChatTest stream、真实 LLM、真实 RAG 或真实认证。
- 当前暂未接入 Docker 部署流程。
