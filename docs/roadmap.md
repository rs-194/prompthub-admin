...
- 当前状态：进行中（已完成对话测试 / Prompt 调试台 v2 knowledge-context mock，支持提示词模板选择、启用模型配置选择、启用中的知识库文档选择、知识库 mock context 预览、mock 输出展示与前端内存测试记录；当前不接后端、不真实调用模型 API、不做真实 RAG；基础参数控制与后续真实调用仍待实现）。
